...
## Goal (phased)
1. Automate ad **audits** (Excel reports already generated by `gen_report.py`).
2. **API data export** — replace manual CSV download from Snapchat Ads Manager with
   a Marketing API pull. ← we are here.
3. Later: write ops — upload creatives, create campaigns / ad sets, change bids.
...
- ⏳ Export module — not written yet. This is your task.
...
## Target CSV schema (export must match this so gen_report.py is unchanged)
Per ad row:
